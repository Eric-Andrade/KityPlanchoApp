    ...
    render() {
        ...
        return (
            <Root>
                ...
                <Title>
                    <TitleText>{ variables.COMPANYNAME } Ayuda</TitleText>
                </Title>
                <ScrollView>
                <ListContainer>
                    <ItemContainer onPress={() =>
                            navigate('InfoScreen', { name: 'Mi primer pedido', text: `Realizar pedidos en ${ variables.COMPANYNAME } es muy sencillo, una vez adquirido una cuenta e iniciado sesión comienzas seleccionado uno de nuestros servicios en la pantalla principal, para luego elegir aquellas prendas las cuales desees que nosotros nos hagamos cargo.
...
Así mismo en cualquiera de los anteriores casos se apoya con un pequeño mensaje al colocar el puntero sobre estos colores. Mismo que funciona en navegadores de computadoras y en algunos teléfonos móviles, esto depende del navegador utilizado y su versión.`})}
                    ...
                    <Separator />
                    <ItemContainer onPress={() =>
                            navigate('InfoScreen', { name: 'Estatus de pedido', text: `Las sucursales de ${ variables.COMPANYNAME } mantienen un horario de servicio corrido de 8:00am a 8:00pm, por tanto el servicio de colecta y entrega se adapta a este mismo, estando sujeto a cambios sin previo aviso por carga laboral. 
Para cualquier aclaración comunícate a los teléfonos y/o correo electrónico brindados en Contáctanos.`})}
                    ...
                    <Separator />
                    <ItemContainer onPress={() =>
                            navigate('InfoScreen', { name: 'Disponibilidad por ubicación', text: `Actualmente ${ variables.COMPANYNAME } labora dentro de La Mancha urbana de la ciudad de Durango Mx. por tanto, facilitamos todos nuestros servicios que se encuentren dentro de la misma.
¿Te gustaría que ${ variables.COMPANYNAME } estuviera en tu ciudad? Háznoslo saber a través de los medios brindados en Contáctanos.`})}
                            >
                        <ItemContainerIcon>
                    ...
                    <Separator />
                    <ItemContainer onPress={() =>
                            navigate('InfoScreen', { name: `Garantía ${ variables.COMPANYNAME }`, text: `¡En ${ variables.COMPANYNAME } lo hacemos con amor! Sí, nos encanta servirte y hacerte sentir mejor liberándote de algunas de las tareas del hogar. Es por ello que transformamos tu confianza en seguridad para tus prendas y nos aseguramos que las tengas de vuelta con lo mejor de nosotros.`})}
                            >
                        <ItemContainerIcon>
                            <Ionicons name="md-heart" size={Platform.OS === 'ios' ? tabIcon : 23} color={colors.PRIMARY}/> 
                        </ItemContainerIcon>
                        <ItemContainerText>
                            <ItemText>Garantía { variables.COMPANYNAME }</ItemText>
                        </ItemContainerText>
                        <ArrowContainer>
                            <Entypo name="chevron-small-right" size={27} color={colors.GRAY600}/> 
                        </ArrowContainer>
